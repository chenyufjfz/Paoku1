...
*/
    ...
    private void LoadObj(string path, bool load_local=false)
    {
    ...
    // only for debug
    private void show_hide_envelop()
    {
    ...
    /*
     * use local model, and the put model in front of enemy
     */
    public void attach_local()
    {
        if (!local_exist)
            return;
        ...
        if (load_state==LoadState.IDLE)
            LoadObj("", true);        
        transform.position = new Vector3(53.51f, 9.26f, -21.4f); //put model in front of enemy
        transform.parent = GameObject.Find("PlayerMesh").transform;        
    }

    /*
     * use model in internet, and the put model in front of enemy
     */
    public void attach_remote()
    {
        string path;
#if UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN
        path = "file://" + Application.streamingAssetsPath + "/model2.obj";
#else
#if UNITY_ANDROID
        path = "file://" + "/sdcard/model.obj";
#endif
#endif
        LoadObj(path, false);
        transform.position = new Vector3(53.51f, 9.26f, -21.4f);//put model in front of enemy
